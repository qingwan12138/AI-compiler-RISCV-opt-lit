...
在 10 个多文件程序上实现预算感知的 module phase ordering，而不是完整复刻 Protean。

### 12.2 输入数据

cBench 中 2–10 个模块的程序。

### 12.3 执行流程

```text
热点模块排序 → 候选 recipe → 不确定性评估 → 双机选择性测量 → 链接复测
```

### 12.4 需要的工具

LLVM、轻量预测器/BO、perf、GNU make、K3。

### 12.5 输出结果

| 方法 | 构建时间 | 真机测量数 | x86/K3 加速 | 代码膨胀 |
|---|---:|---:|---:|---:|

### 12.6 成功标准

在显著更低调优时间下接近或超过固定轮数的模块搜索，并控制跨模块退化。
