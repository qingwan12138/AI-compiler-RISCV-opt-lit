...
对 20 个程序构建 pass 级反事实证据图，并与无证据 GA/CITROEN 比较。

### 12.2 输入数据

cBench、PolyBench，候选序列限制为 5–10 个 pass。

### 12.3 执行流程

```text
生成短序列 → 单pass删减/相邻交换 → 双机计时 → 证据图 → 引导搜索
```

### 12.4 需要的工具

LLVM、perf、K3、统计检验、轻量搜索模型。

### 12.5 输出结果

| 方法 | 测量预算 | 证据一致率 | x86/K3 加速 | 可解释覆盖率 |
|---|---:|---:|---:|---:|

### 12.6 成功标准

在相同预算下优于无证据搜索，并能用复现实验支持大多数关键 pass 判断。
