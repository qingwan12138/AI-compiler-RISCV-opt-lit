...
修复 30 个 TorchDynamo graph break，并测试 CPU/GPU 两后端。

### 12.2 输入数据

ParityBench 中可稳定复现的 30 个失败样本。

### 12.3 执行流程

```text
日志归因 → 生成 3 个修复 → 双后端比较测试 → 测量图数/时间/显存
```

### 12.4 需要的工具

PyTorch 2.5、TorchDynamo/Inductor、CPU/GPU、代码 LLM。

### 12.5 输出结果

| 样本 | 修复成功 | CPU 加速 | GPU 加速 | 数值误差 |
|---|---:|---:|---:|---:|

### 12.6 成功标准

大多数样本在双后端均正确，并减少图数且至少一端获得稳定加速。
