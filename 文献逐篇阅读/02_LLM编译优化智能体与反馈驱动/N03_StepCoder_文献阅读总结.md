...
验证“只反馈执行覆盖行”能否提高错误修复成功率。

### 12.2 输入数据

20 个含死代码和分支的 Python 小题。

### 12.3 执行流程

```text
生成错误代码 → coverage.py 标记执行行 → 比较全量反馈与覆盖行反馈 → 重试修复
```

### 12.4 需要的工具

Python、coverage.py、pytest、LLM API。

### 12.5 输出结果

| 方法 | 编译成功率 | 测试通过率 | 平均重试次数 |
|---|---:|---:|---:|

### 12.6 成功标准

覆盖反馈在相同调用预算下提高测试通过率或减少重试。
