...
在 K3 上实现量化 GEMV 的 LLVM/RVV 替换原型。

### 12.2 输入数据

1B 模型的代表 GEMV 尺寸和随机权重。

### 12.3 执行流程

```text
C 朴素 GEMV → LLVM Pass → RVV/BLAS kernel → K3 测时间/能耗
```

### 12.4 需要的工具

LLVM、RISC-V 工具链、RVV intrinsic、K3、功耗采集工具。

### 12.5 输出结果

| 方案 | 内存 | tok/s/GEMV 时间 | 能耗 | 误差 |
|---|---:|---:|---:|---:|

### 12.6 成功标准

联合方案同时优于仅量化和仅 RVV kernel。
