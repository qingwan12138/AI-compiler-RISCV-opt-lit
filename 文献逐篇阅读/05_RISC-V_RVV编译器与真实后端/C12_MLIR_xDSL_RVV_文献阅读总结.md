...
在 K3 上打通一个 FP32 GEMM 的 xDSL/MLIR→RVV→C++ 流程，并自动选择优于固定 baseline 的微内核。

### 12.2 输入数据

5 个方阵形状和论文中的 5 个 BERT-Large GEMM 形状。

### 12.3 执行流程

```text
生成 20–50 个候选 → 编译 → 正确性检查
→ K3 计时/PMU → 选择 → 与 OpenBLAS、Clang 自动向量化比较
```

### 12.4 需要的工具

xDSL、LLVM/MLIR、RISC-V GCC/Clang、RVV intrinsic、K3 开发板、性能计数工具。

### 12.5 输出结果

| 形状 | 固定内核 | 自动选择 | OpenBLAS | 最优配置 | 加速比 |
|---|---:|---:|---:|---|---:|

### 12.6 成功标准

所有形状数值正确；自动选择平均优于固定配置，并在至少一组真实负载上稳定超过 OpenBLAS 或编译器自动向量化基线。
