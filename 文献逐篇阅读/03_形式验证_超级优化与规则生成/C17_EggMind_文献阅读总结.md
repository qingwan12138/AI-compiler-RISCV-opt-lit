...
在 10–15 个代数/向量表达式上比较 full EqSat、专家两阶段策略和小型 LLM 生成 EqSatL 策略。

### 12.2 输入数据

20 条带语义标签的规则、5 个演化样本、5–10 个 held-out 样本，故意加入会双向膨胀的 expose 规则。
...
→ 记录成本、节点数、内存和超时
→ 抽取短规则链作为 motif
→ 迭代 10–20 次 → held-out 比较
```

### 12.4 需要的工具

egg/egglog 或 Foresight、Python、可用 LLM API；RVV 扩展再加入 LLVM、QEMU/真机。

### 12.5 输出结果

| 样本 | 策略 | 最终成本 | 峰值节点 | 时间 | 内存 | 是否超时 |
|---|---|---:|---:|---:|---:|---|

### 12.6 成功标准

LLM 策略在 held-out 样本上优于 full EqSat 的资源—质量折中，且所有策略通过 schema 和等价规则检查。
