...
对 1–5 万个含整数/向量指令的 LLVM IR 切片发现并验证候选，筛选在 K3 上确有收益者。

### 12.2 输入数据

PolyBench、OpenCV 或 RVV 相关程序的优化后 LLVM IR。

### 12.3 执行流程

```text
切片 → LLM 两轮候选 → opt/Alive2 → RV64 汇编 → K3 计时
```

### 12.4 需要的工具

LLVM、Alive2、代码 LLM、RISC-V 后端、K3。

### 12.5 输出结果

| 阶段 | 候选数 | 通过率 | 新规则数 | K3 实测收益 |
|---|---:|---:|---:|---:|

### 12.6 成功标准

至少得到若干 Alive2 证明、LLVM 当前未实现且 K3 重复测量有稳定收益的候选，并说明泛化范围。
