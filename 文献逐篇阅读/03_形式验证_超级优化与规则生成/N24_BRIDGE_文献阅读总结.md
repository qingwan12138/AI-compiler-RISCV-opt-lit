...
比较 GCC/Clang x86/RISC-V 二进制提升结果的一致性。

### 12.2 输入数据

100 个纯 C 函数，四种编译器/架构组合。

### 12.3 执行流程

```text
反汇编 → 结构 RAG → IR → 编译/差分 → 规范化相似度
```

### 12.4 需要的工具

LLVM/GCC、QEMU/K3、Alive2/KLEE、代码 LLM。

### 12.5 输出结果

| 组合 | 可编译 | 可执行 | 跨编译器一致 | 验证未知 |
|---|---:|---:|---:|---:|

### 12.6 成功标准

在保持可执行率时显著提高同源函数的规范 IR 一致性。
