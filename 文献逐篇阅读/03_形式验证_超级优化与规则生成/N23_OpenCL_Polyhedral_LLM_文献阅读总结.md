...
自动转换 5 个 PolyBench/NPDP kernel 到 OpenCL，并统计无需人工修改率。

### 12.2 输入数据

Pluto 生成的 5 个 OpenMP tiled 程序。

### 12.3 执行流程

```text
生成 OpenCL → Clang 反馈修复 → CPU/GPU 差分 → 测时
```

### 12.4 需要的工具

Pluto/isl、Clang/OpenCL、CPU/GPU、代码 LLM。

### 12.5 输出结果

| Kernel | 自动编译 | 差分通过 | 修复轮数 | GPU 加速 |
|---|---:|---:|---:|---:|

### 12.6 成功标准

大多数 kernel 无人工修改地通过差分并获得稳定加速。
